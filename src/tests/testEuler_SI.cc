...
TEST_CASE("Euler_SI Single Particle", "[integrator]")
{
  //Create a particle
  Particles particles ({
    Particle (1., 1.)});
  ...
  REQUIRE(particles[0].x == x0 + ax_0*dt*dt);
  REQUIRE(particles[0].y == vy0*dt + ay_0*dt*dt);
  REQUIRE(particles[0].z == az_0*dt*dt);
  ...
  //step doesn't allow Euler_SI method to converge on analytic answer
  //very well
  for (int i=2; i<52; ++i){
  ...
  CHECK( (particles[0].x - (x0 + .5*ax_0*(dt*maxint)*(dt*maxint))) <= 2e-8);
  CHECK( (particles[0].y - (vy0*(dt*maxint) + .5*ay_0*(dt*maxint)*(dt*maxint))) < 2e-8);
  CHECK( (particles[0].z - (.5 * az_0*(dt*maxint)*(dt*maxint))) < 2e-8);
  
  //Analytic solution of v(t) with constant acceleration:
  // v_f = v_0 + a * t
  CHECK( (particles[0].vx - ax_0*(dt*maxint)) <= 2e-8);
  CHECK( (particles[0].vy - (vy0 + ay_0*(dt*maxint))) <= 2e-8);
  CHECK( (particles[0].vz - az_0*(dt*maxint)) <= 2e-8);
...
TEST_CASE("Euler_SI Many Particle", "[integrator]"){
  Particles particles;
