...
TEST_CASE("RungeKutta4 Single Particle", "[integrator]")
{
  //Create a particle
  Particles particles ({
    Particle (1., 1.)});
  ...
  //step doesn't allow RungeKutta4 method to converge on analytic answer
  //very well
  for (int i=2; i<52; ++i){
...
TEST_CASE("RungeKutta4 Many Particle", "[integrator]"){
  Particles particles;
