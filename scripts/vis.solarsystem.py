...
mdata = create_sphere(20, 40, .001)
spherelist = [create_sphere(20, 40, .002), mdata, mdata, mdata, mdata]
colorlist = ['y','silver','gold','b','r']
    ...
    def __init__(self, data):
        ...
        for x,y,z in np.vstack(np.split(self.d[self.i],self.np))[:,:3]:
            m = scene.visuals.Mesh(
                meshdata=spherelist[i], color=colorlist[i], shading='smooth')
